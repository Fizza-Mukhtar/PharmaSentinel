// src/pages/dashboard/manufacturer/CreateBatch.js
...
export default function CreateBatch() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios
      .post("http://localhost:8000/api/medicine/batches/", form, {
        headers: { Authorization: `Token ${localStorage.getItem("token")}` },
      })
      .then((res) => setMsg({ type: "success", text: "Batch created!" }))
      .catch((err) =>
        setMsg({ type: "danger", text: err.response?.data?.detail || "Error!" })
      );
  };
