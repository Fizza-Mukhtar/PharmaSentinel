...
export default function DistributorVerifyBatch() {
  ...
  const getStatusIcon = (result) => {
    if (!result) return <FiShield size={48} />;
    const severity = result.severity;
    if (severity === 'CRITICAL') return <FiAlertOctagon size={48} />;
    if (severity === 'HIGH' || severity === 'MEDIUM') return <FiAlertTriangle size={48} />;
    return result.valid && result.authentic ? <FiCheckCircle size={48} /> : <FiAlertOctagon size={48} />;
  };

  return (
    <DistributorLayout>
      <div className="mb-4">
        <h2 className="mb-2 fw-bold d-flex align-items-center gap-2">
          <FiShield style={{ color: '#047857' }} />
          Verify Medicine Batch
        </h2>
        <p className="text-muted mb-0">Verify authenticity and blockchain integrity of medicine batches</p>
      </div>
      ...
      <Card className="border-0 shadow-sm mb-4">
        <Card.Body className="p-4">
          ...
          <Alert variant="info" className="mt-3 mb-0 border-0 bg-light">
            <FiInfo className="me-2" />
            <small>
              Enter a batch ID or select from dropdown to verify authenticity and view supply chain trail
            </small>
          </Alert>
        </Card.Body>
      </Card>
      ...
      {result && (
        <>
              ...
              <div className="d-flex align-items-center gap-4">
                ...
                <div className="flex-grow-1">
                  <h3 className="fw-bold mb-2" style={{ color: getSeverityColor(result) }}>
                    {result.title || result.status || "Verification Complete"}
                  </h3>
            ...
            <Card className="border-0 shadow-sm mb-4">
              <Card.Body>
                ...
                <Row>
                  ...
                  <Col md={4} className="mb-3">
                    <div className="p-3 border rounded" style={{ backgroundColor: "#f9fafb" }}>
                      <small className="text-muted fw-semibold d-block mb-1">MANUFACTURED ON</small>
                      <div className="fw-bold mt-1">{result.medicine.manufactured_on}</div>
                    </div>
                  </Col>
                  <Col md={4} className="mb-3">
                    <div className="p-3 border rounded" style={{ backgroundColor: "#f9fafb" }}>
                      <small className="text-muted fw-semibold d-block mb-1">EXPIRES ON</small>
                ...
                <Row>
                  ...
                  {result.batch_info.expiry_date && (
                    <Col md={6} className="mb-3">
                      ...
                      <strong>{result.batch_info.expiry_date}</strong>
                    </Col>
                  )}
                </Row>
              </Card.Body>
          ...
          {/* Detailed Info (for authenticated users) */}
                ...
                <Row>
                  <Col md={3} className="mb-3">
                    <div className="p-3 border rounded" style={{ backgroundColor: "#f9fafb" }}>
                      ...
                      <Badge 
                        bg={result.detailed_info.batch_status === 'ACTIVE' ? 'success' : 'danger'} 
                        className="mt-1"
                      >
                  ...
                  <Col md={3} className="mb-3">
                    <div className="p-3 border rounded" style={{ backgroundColor: "#f9fafb" }}>
                      ...
                      <strong className="d-block mt-1">
                        {result.detailed_info.total_manufactured} units
                      </strong>
                    </div>
                  </Col>
                  <Col md={3} className="mb-3">
                    <div className="p-3 border rounded" style={{ backgroundColor: "#f9fafb" }}>
                      ...
                      <strong className="d-block mt-1">
                        {result.detailed_info.total_distributed} units
                      </strong>
                    </div>
                  </Col>
                  <Col md={3} className="mb-3">
                    <div className="p-3 border rounded" style={{ backgroundColor: "#f9fafb" }}>
                      <small className="text-muted fw-semibold d-block mb-1">IN SUPPLY CHAIN</small>
                      <strong className="d-block mt-1">
                        {result.detailed_info.remaining_in_supply_chain} units
                      </strong>
                    </div>
                  </Col>
          ...
          {/* Blockchain Trail (for authenticated users) */}
            ...
            <Card className="border-0 shadow-sm">
              <Card.Body>
                <div className="d-flex justify-content-between align-items-center mb-4">
                  ...
                  <Badge 
                    bg={result.detailed_info.blockchain.is_valid ? "success" : "danger"} 
                    className="px-3 py-2"
                  >
                ...
                <Alert variant="info" className="mb-3">
                  <FiInfo className="me-2" />
                  <strong>Total Blocks:</strong> {result.detailed_info.blockchain.total_blocks}
                  {' | '}Each block represents a verified transaction
                </Alert>
                <div className="position-relative" style={{ paddingLeft: "40px" }}>
