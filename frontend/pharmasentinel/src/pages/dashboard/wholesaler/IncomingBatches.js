// src/pages/wholesaler/IncomingBatches.js
...
import { getWholesalerIncoming , acceptWholesalerTransfer , rejectBatch } from "../../../services/api";
...
export default function IncomingBatches() {
  ...
  const loadIncomingBatches = async () => {
    ...
    setError(null);
    try {
      const data = await getWholesalerIncoming();
  ...
  const confirmAccept = async () => {
    ...
    setSuccess(null);
    try {
      await acceptWholesalerTransfer(selectedBatch.transfer_id);
      setSuccess(`✅ Batch ${selectedBatch.batch_id} accepted successfully!`);
  ...
  const confirmReject = async () => {
    ...
    setSuccess(null);
    try {
      await rejectBatch(selectedBatch.transfer_id, rejectReason); // API call
      setSuccess(`❌ Batch ${selectedBatch.batch_id} rejected. Reason: ${rejectReason}`);
  ...
  return (
    <WholesalerLayout>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="mb-1">📥 Incoming Batches</h2>
          <p className="text-muted mb-0">Review and accept batches from warehouses.</p>
        </div>
        <Button variant="outline-primary" onClick={loadIncomingBatches} disabled={loading || actionLoading}>
          🔄 Refresh
        </Button>
      </div>
        ...
        <Modal.Body>
          ...
          {selectedBatch && (
            <div className="bg-light p-3 rounded">
              ...
              <p><strong>From:</strong> {selectedBatch.from_user}</p>
            </div>
          )}
          <Alert variant="info" className="mt-3 mb-0">
            <small>ℹ️ A blockchain confirmation block will be created upon acceptance.</small>
          </Alert>
        </Modal.Body>
        <Modal.Footer>
