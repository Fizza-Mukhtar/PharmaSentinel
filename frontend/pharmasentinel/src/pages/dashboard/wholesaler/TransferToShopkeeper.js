// src/pages/wholesaler/TransferToWholesaler.js
...
import { 
  ...
  getShopkeeperUsers, 
  transferToShopkeeper 
} from "../../../services/api";

export default function TransferToShopkeeper() {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [inventoryData, shopkeepersData] = await Promise.all([
        getWholesalerInventory(),
        getShopkeeperUsers(),
      ]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await transferToShopkeeper(formData);
  ...
  return (
    <WholesalerLayout>
      <div className="d-flex justify-content-between align-items-center mb-4">
        <div>
          <h2 className="mb-1">🔄 Transfer to Shopkeeper</h2>
          <p className="text-muted mb-0">Transfer medicine batches to shopkeeper.</p>
        </div>
        <Button 
          ...
          <Card className="border-0 shadow-sm">
            <Card.Body>
              ...
              <Form onSubmit={handleSubmit}>
                ...
                <Form.Group className="mb-3">
                  <Form.Label>Select Shopkeeper *</Form.Label>
                  <Form.Select
                  ...
                  >
                    ...
                    {shopkeepers.map((shopkeeper) => (
                      <option key={shopkeeper.id} value={shopkeeper.id}>
                        {shopkeeper.username} (ID: {shopkeeper.id})
                      </option>
                    ))}
                  </Form.Select>
                  {shopkeepers.length === 0 && (
                    <Form.Text className="text-danger">
                      No shopkeeper available
                    </Form.Text>
                  )}
                </Form.Group>
                ...
                <div className="d-flex gap-2">
                  <Button
                    type="submit"
                    variant="primary"
                    disabled={loading || !formData.batch_id || !formData.receiver_id || !formData.quantity}
                  >
