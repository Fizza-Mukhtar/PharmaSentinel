...
import { 
  FaWarehouse, FaSearch, FaFilter, FaBoxes, FaCheckCircle,
  FaExchangeAlt, FaCalendar, FaEnvelope, FaDownload
} from 'react-icons/fa';
import { getDrapWarehouses } from '../../../services/api';
...
const Warehouses = () => {
  ...
  const fetchWarehouses = async () => {
    try {
      setLoading(true);
      const response = await getDrapWarehouses();
  ...
  const stats = {
    ...
    totalInventory: warehouses.reduce((sum, w) => sum + (w.inventory_items || 0), 0),
    totalReceived: warehouses.reduce((sum, w) => sum + (w.transfers_received || 0), 0),
    totalSent: warehouses.reduce((sum, w) => sum + (w.transfers_sent || 0), 0)
  };
      ...
      <div className="stakeholder-header">
        <div>
          ...
          <p className="stakeholder-subtitle">
            Monitor all registered warehouse facilities
          </p>
        </div>
                    ...
                    <tr key={warehouse.id}>
                      ...
                      <td className="text-center">
                        <Badge bg="primary" className="metric-badge">
                          ...
                          {warehouse.inventory_items || 0}
                        </Badge>
                      </td>
                      <td className="text-center">
                        <Badge bg="success" className="metric-badge">
                          {warehouse.transfers_received || 0}
                        </Badge>
                      </td>
                      <td className="text-center">
                        <Badge bg="info" className="metric-badge">
                          {warehouse.transfers_sent || 0}
                        </Badge>
                      </td>
                      <td className="text-center">
                        <Badge 
                          bg={warehouse.is_active ? 'success' : 'danger'}
                      ...
                      <td className="text-center">
                        ...
                        {new Date(warehouse.date_joined).toLocaleDateString()}
                      </td>
                    </tr>
                  ))
                )}
